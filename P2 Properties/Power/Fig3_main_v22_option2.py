...
def draw_panel_a(ax, latency_series):
    ...
    leg = ax.legend(frameon=False, loc='lower left', bbox_to_anchor=(0.0, 1.01), ncol=2,
                    handlelength=1.3, columnspacing=0.9, borderaxespad=0.0)
...
def annotate_point_labels(ax, xs, ys, labels, color, mode='lat'):
    offsets = {'64×64': (-10, 5), '128×128': (0, 6), '256×256': (8, -2), '64x64': (-10, 5), '128x128': (0, 6), '256x256': (8, -2)}
    for x, y, lab in zip(xs, ys, labels):
        dx, dy = offsets.get(str(lab), (4, 4))
        short = str(lab).replace('×', '×')
        ax.annotate(short, (x, y), textcoords='offset points', xytext=(dx, dy), ha='center', va='bottom', fontsize=4.2, color=color)
...
def draw_panel_f(ax, df):
    ...
    for s in ax.spines.values():
        s.set_visible(False)
    ax_lat = ax.inset_axes([0.03, 0.17, 0.43, 0.75])
    ax_pow = ax.inset_axes([0.53, 0.17, 0.43, 0.75])
    plot_order = ['This work', 'Jetson Orin NX', 'CPU i7-13700', 'RTX 4090']
    ticks, ticklabels = [], []
    for name in plot_order:
        g = df[df['platform'] == name].sort_values('pixels')
        ...
        ax_lat.plot(px, lat, marker=meta['marker'], color=meta['color'], lw=1.0, ms=PANEL_DEF_MARKER_MS, label=name)
        ax_pow.plot(px, pw, marker=meta['marker'], color=meta['color'], lw=1.0, ms=PANEL_DEF_MARKER_MS, label=name)
        annotate_point_labels(ax_lat, px.to_numpy(), lat.to_numpy(), lab, meta['color'])
        annotate_point_labels(ax_pow, px.to_numpy(), pw.to_numpy(), lab, meta['color'])
    ...
    ax_lat.set_yscale('log'); ax_pow.set_yscale('log')
    ax_lat.set_ylabel('Latency (ms)')
    ax_pow.set_ylabel('Power (W)')
    add_platform_legend(ax, loc='upper left', ncol=4, bbox_to_anchor=(0.01, 0.99))
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v21_refined.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v21_refined.png'
