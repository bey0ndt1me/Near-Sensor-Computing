...
\textbf{a,} Paradigm comparison between conventional host-dependent processing (left), in which raw video is transferred to a host CPU or GPU for software reconstruction, and the near-sensor architecture presented here (right), which reconstructs each depth map on-chip at the sensing interface with a fixed latency of 0.211\,ms.
\textbf{b,} Schematic of the visuotactile sensor, showing the soft elastomer dome, optical window, ring of red, green and blue light sources and OV5645 CMOS imager.
\textbf{c,} Simplified block diagram of the streaming reconstruction pipeline operating at 166~MHz, comprising a photometric-stereo core and a spectral Poisson core; the end-to-end latency is fixed at 35{,}107~clock cycles (0.211\,ms), independent of contact geometry.
\textbf{d,} Representative depth maps reconstructed on-chip at successive stages of a single press of a \placeholder{XXX} indenter into the elastomer, illustrating the dense contact geometry recovered directly at the sensing interface.}
\label{fig:system}
\end{figure}
