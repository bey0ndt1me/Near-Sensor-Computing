...
def load_panel_d_energy_throughput():
    """Load panel-d data as FULL-CHIP energy per frame.

    Required columns in template/fig3d_energy_throughput_template.csv:
        platform, target_fps, processed_fps

    Preferred full-chip columns:
        full_chip_power_w, full_chip_energy_per_frame_mj

    Compatibility:
        If full_chip_power_w is absent, active_power_w is treated as full-chip
        active power for measured CPU/GPU/Jetson rows. Incremental power/energy
        columns are intentionally ignored here and should be plotted in Extended
        Data instead.
    """
    df = read_csv(FILE_D)
    if df.empty:
    ...
    if 'full_chip_power_w' not in df.columns:
        ...
        elif 'device_power_w' in df.columns:
            df['full_chip_power_w'] = df['device_power_w']
        else:
    ...
    m = (
        df['full_chip_energy_per_frame_mj'].isna()
        & df['full_chip_power_w'].notna()
        & df['processed_fps'].notna()
    )
    df.loc[m, 'full_chip_energy_per_frame_mj'] = (
        1000.0 * df.loc[m, 'full_chip_power_w'].astype(float)
        / df.loc[m, 'processed_fps'].astype(float)
    )

    if 'bubble_size_w' not in df.columns:
        df['bubble_size_w'] = df['full_chip_power_w']
    else:
        df['bubble_size_w'] = df['bubble_size_w'].fillna(df['full_chip_power_w'])

    if 'point_id' not in df.columns:
        df['point_id'] = df['platform']
    if 'energy_basis' not in df.columns:
        df['energy_basis'] = 'full-chip'
    return df
...
def draw_panel_c(ax, df):
    ...
    labels = ['This work', 'Jetson Orin NX', 'CPU i7-13700', 'GPU RTX 4090']
    x = np.arange(4)
    ...
    # Make the dark-segment visual length equal to the core-power share rather than
    # the raw core wattage. On a log axis, this requires positioning the split in
    # log space while preserving the true total power as the bar top.
    y_base = 1e-2
    ymax = y_base
    for i, p in enumerate(plot_order):
        ...
        total = core + bg
        core_frac = core / total if total > 0 else 0.0
        split_y = y_base * (total / y_base) ** core_frac if total > y_base else total

        ax.bar(x[i], split_y - y_base, bottom=y_base, width=0.48,
               color=dark[i], edgecolor='white', linewidth=0.6)
        ax.bar(x[i], total - split_y, bottom=split_y, width=0.48,
               color=light[i], edgecolor='white', linewidth=0.6)

        total_label = f'{total*1000:.2f} mW' if total < 1 else (f'{total:.2f} W' if total < 10 else f'~{total:.0f} W')
        pct_label = f'{core_frac*100:.0f}%'
        ax.text(x[i], total * 1.10, total_label, ha='center', va='bottom', fontsize=4.8)
        ax.text(x[i], split_y * 1.06, pct_label, ha='center', va='bottom', fontsize=4.7, color=COLOR_TEXT)
        ymax = max(ymax, total)
    ...
    ax.text(0.02, 0.98, 'Dark segment length encodes the core-compute share; light indicates background/system.', transform=ax.transAxes, ha='left', va='top', fontsize=4.7, bbox=dict(boxstyle='round,pad=0.18', facecolor='white', edgecolor='0.85'))
...
def draw_panel_d(ax, df):
    ax.set_title('Full-chip energy per frame versus throughput', loc='left', fontweight='bold', pad=6)
    strip_axes(ax)
    ax.grid(color=COLOR_GRID, linewidth=0.55, which='both')
    ax.set_axisbelow(True)

    df = df.copy()
    if 'energy_basis' in df.columns:
        df = df[~df['energy_basis'].astype(str).str.lower().str.contains('incremental', na=False)]
    ...
    ax.annotate('Preferred', xy=(0.86, 0.10), xytext=(0.68, 0.22),
                textcoords='axes fraction', xycoords='axes fraction', fontsize=4.9,
                color='0.30', arrowprops=dict(arrowstyle='->', lw=0.7, color='0.45'))
    ...
    ax.text(0.02, 0.02, 'All markers are shown at a uniform size for visual comparability.', transform=ax.transAxes,
...
def draw_panel_e(ax, df):
    ...
    for name in plot_order:
        ...
        if len(g_128):
            row = g_128.iloc[0]
            ax.scatter([row['latency_ms']], [row['power_w']], s=PANEL_DEF_SCATTER_S, marker=meta['marker'], color=meta['color'], edgecolor='k', linewidth=0.55, zorder=4)
...
def draw_panel_f(ax, df):
    ...
    for name in plot_order:
        ...
        if g.empty:
            continue
        meta = PLATFORM_META[name]
        ax_lat.plot(g['pixels'], g['latency_ms'], marker=meta['marker'], color=meta['color'], lw=1.1, ms=PANEL_DEF_MARKER_MS, label=name)
        ax_pow.plot(g['pixels'], g['power_w'], marker=meta['marker'], color=meta['color'], lw=1.1, ms=PANEL_DEF_MARKER_MS, label=name)
...
def add_caption(fig):
    caption = (
        ...
        'd, Full-chip energy per frame versus throughput. The vertical dashed line marks the 180 fps real-time requirement; points to the lower right are preferred. '
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v19_panelc_fraction_split.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v19_panelc_fraction_split.png'
