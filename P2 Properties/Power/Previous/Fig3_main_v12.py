...
def load_panel_d_energy_throughput():
    ...
    # platform, point_id, target_fps, processed_fps, active_power_w, idle_power_w, incremental_power_w, energy_per_frame_mj, latency_p50_ms, latency_p95_ms, latency_p99_ms, meets_180_fps, notes
...
def draw_panel_d(ax, df):
    ...
    strip_axes(ax)
    ax.grid(color=COLOR_GRID, linewidth=0.55)
    ax.set_axisbelow(True)
    ...
    ax.axvspan(0, 180, color='0.96', zorder=0)
    ax.axvline(180, color='0.42', lw=0.95, ls='--', zorder=1)
    ax.text(180, 0.04, '180 FPS real-time requirement', transform=ax.get_xaxis_transform(),
            ha='center', va='bottom', fontsize=4.9, color='0.36')
    ax.text(90, 0.96, 'cannot meet real-time', transform=ax.get_xaxis_transform(),
            ha='center', va='top', fontsize=4.8, color='0.56')

    color_map = {'ASIC full-chip': COLOR_THISWORK, 'Jetson Orin NX': COLOR_JETSON,
                 'CPU i7-13700': COLOR_CPU, 'RTX 4090': COLOR_GPU}
    marker_map = {'ASIC full-chip': 'o', 'Jetson Orin NX': '^', 'CPU i7-13700': 'D', 'RTX 4090': 's'}
    ...
    for name in label_order:
        ...
        ax.plot(xs, ys, color=color_map[name], lw=0.95, alpha=0.65, zorder=2)
        ax.scatter(xs, ys, s=ss, marker=marker_map[name], color=color_map[name], edgecolor='white', linewidth=0.45, alpha=0.95, zorder=3)
        xvals.extend(xs.tolist()); yvals.extend(ys.tolist())
        endpoint_rows.append((name, xs[-1], ys[-1]))

    # direct labels near the highest-throughput points
    label_spec = {'ASIC full-chip': (8, 1.03), 'Jetson Orin NX': (8, 1.00), 'CPU i7-13700': (8, 1.02), 'RTX 4090': (8, 0.96)}
    for name, x, y in endpoint_rows:
        dx, ymul = label_spec.get(name, (8, 1.0))
        ax.text(x + dx, y * ymul, name, fontsize=4.9, color=color_map[name], ha='left', va='center')

    # subtle callout for the desired operating region
    ax.annotate('Preferred', xy=(0.86, 0.10), xytext=(0.72, 0.22), textcoords='axes fraction', xycoords='axes fraction',
                fontsize=4.9, color='0.30', arrowprops=dict(arrowstyle='->', lw=0.7, color='0.45'))

    xmax = max(220, max(xvals) * 1.10 if xvals else 220)
    ymin = max(0.08, min(yvals) * 0.75 if yvals else 0.08)
    ymax = max(3.0, max(yvals) * 1.35 if yvals else 3.0)
    ax.set_xlim(0, xmax)
    ax.set_ylim(ymin, ymax)
    ax.set_yscale('log')
    ax.set_xlabel('Processed FPS')
...
def draw_panel_e(ax, df):
    ...
    ax.legend(handles=[Line2D([0], [0], marker='o', color=COLOR_THISWORK, markersize=4.8, lw=1.0, label='This work'), Line2D([0], [0], marker='o', color=COLOR_JETSON, markersize=4.8, lw=1.0, label='Jetson Orin NX'), Line2D([0], [0], marker='D', color=COLOR_CPU, markersize=4.6, lw=1.0, label='CPU i7-13700'), Line2D([0], [0], marker='s', color=COLOR_GPU, markersize=4.8, lw=1.0, label='RTX 4090'), Line2D([0], [0], color=COLOR_FRONTIER, lw=1.2, linestyle='--', label='Pareto frontier')], frameon=False, loc='lower right')
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v12.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v12.png'
