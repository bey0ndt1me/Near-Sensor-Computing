...
FILE_A = TEMPLATE_DIR / 'fig3a_latency_samples_template.csv'
FILE_B = TEMPLATE_DIR / 'fig3b_pipeline_breakdown_template.csv'
FILE_C = TEMPLATE_DIR / 'fig3c_power_split_template.csv'
FILE_D = TEMPLATE_DIR / 'fig3d_pareto_points_template.csv'
FILE_E = TEMPLATE_DIR / 'fig3e_resolution_scaling_template.csv'
...
def read_csv(path):
    ...
    last_error = None
    for encoding in ('utf-8-sig', 'cp1252', 'gb18030', 'latin1'):
        try:
        ...
        raise UnicodeDecodeError(
            ...
            last_error.start,
            last_error.end,
            f'Could not decode {path} with utf-8-sig, cp1252, gb18030, or latin1: {last_error.reason}',
        )
...
def load_panel_a_latency():
    ...
    if 'CPU i7-12700' in series:
        series['CPU workstation (i7-12700)'] = series.pop('CPU i7-12700')
...
def load_panel_c_power():
    ...
    # fallback totals
    defaults = {'This work': THISWORK_POWER_W, 'Jetson Orin NX': JETSON_POWER_W, 'CPU i7-12700': CPU_POWER_W, 'RTX 4090': GPU4090_POWER_W}
...
def load_panel_d_pareto():
    df = remove_512_operating_points(fill_operating_metrics(read_csv(FILE_D)))
...
def draw_panel_a(ax, latency_series):
    ...
    for key, ytxt, xmul, col in [('Jetson Orin NX', 0.90, 1.06, COLOR_JETSON), ('GPU workstation (RTX 4090)', 0.83, 1.04, COLOR_GPU), ('CPU workstation (i7-12700)', 0.96, 1.03, COLOR_CPU)]:
...
def draw_panel_d(ax, df):
    ...
    ax.set_axisbelow(True)
    short_name = {'This work': 'FPGA', 'Jetson Orin NX': 'Jetson', 'CPU i7-12700': 'CPU', 'RTX 4090': '4090'}
    plot_order = ['This work', 'Jetson Orin NX', 'CPU i7-12700', 'RTX 4090']
...
def draw_panel_e(ax, df):
    ...
    ax.text(0.02, 0.04, 'Panel e reads the fig3e CSV directly; fill real scaling data here.', transform=ax.transAxes, ha='left', va='bottom', fontsize=4.6, color='0.35')
...
def main():
    ...
    d_df = load_panel_d_pareto()
    e_df = load_panel_e_scaling()
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v8.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v8.png'
