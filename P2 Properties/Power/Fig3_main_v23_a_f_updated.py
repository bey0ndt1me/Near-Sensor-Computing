...
def annotate_numeric_labels(ax, xs, ys, texts, color):
    offsets = [(-8, 4), (0, 6), (8, -2), (-10, -8), (10, 4)]
    for i, (x, y, txt) in enumerate(zip(xs, ys, texts)):
        dx, dy = offsets[i % len(offsets)]
        ax.annotate(txt, (x, y), textcoords='offset points', xytext=(dx, dy),
                    ha='center', va='bottom', fontsize=4.1, color=color)
...
def draw_panel_f(ax, df):
    ...
    for name in plot_order:
        ...
        annotate_numeric_labels(ax_lat, px, lat, lat_texts, meta['color'])
        annotate_numeric_labels(ax_pow, px, pw, pow_texts, meta['color'])
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v23_a_f_updated.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v23_a_f_updated.png'
