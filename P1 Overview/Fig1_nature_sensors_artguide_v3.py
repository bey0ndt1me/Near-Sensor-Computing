...
图 1 绘图脚本说明
------------------
这份脚本用 matplotlib 手动排版 Nature 系列期刊风格的多 panel 图。
最常改的地方：
1. 整张图尺寸：改 FIG_W、FIG_H。
2. 每个 panel 的位置和大小：改 make_figure() 里的 fig.add_axes([left, bottom, width, height])。
   四个数都是 0-1 的画布比例坐标。
3. panel 内某张图片或文字的位置：进入对应 draw_panel_* 函数，改 x/y 坐标或 box=(x, y, w, h)。
4. 图片物理尺寸：通常由 axes_size_mm(ax, 宽mm, 高mm) 控制。
5. 箭头长度：改 centered_gap_arrow(..., length=...)。
6. 真实图片素材：放到 ./materials/ 或脚本同目录，文件名见下方。

Nature 系列投稿相关约束：
- 当前目标宽度为 180 mm，适合双栏图。
- 字体使用 sans-serif，显式字号尽量不低于 5 pt。
- 图片保持原始宽高比，不拉伸。
- 图内不放整段 caption，只保留必要标注。

可选素材文件：
  GelSight.png, Sensor.png, Sensor_Explosion.png, raw_rgb.png, my_depth_2d.png
  depth_sequence/depth_000.png ... depth_sequence/depth_005.png

输出文件：
  Fig1_nature_sensors_artguide.svg  (主格式: 矢量可编辑)
  Fig1_nature_sensors_artguide.pdf  (投稿必需)
  Fig1_nature_sensors_artguide.tiff (Nature 600 dpi)
  Fig1_nature_sensors_artguide.png  (快速预览)
"""
...
# ------------------------------ 全局风格 ------------------------------
# 这里控制字体、字号、线宽、导出分辨率。
# 如果整张图文字偏小，优先改 font.size；如果线条太粗或太细，改 axes.linewidth。
mpl.rcParams.update({
    # ── MANDATORY: editable SVG/PDF text (Nature submission required) ──
    "font.family": "sans-serif",
    "font.sans-serif": ["Arial", "Helvetica", "DejaVu Sans", "sans-serif"],
    "svg.fonttype": "none",        # SVG 文字保持为 <text> 节点，可编辑
    "pdf.fonttype": 42,            # PDF 文字保持 TrueType，可编辑
    "ps.fonttype": 42,
    # ── Nature style defaults ────────────────────────────────────────
    "font.size": 6.0,              # Nature 双栏图建议 5–7 pt，6 pt 更保险
    "axes.linewidth": 0.55,
    "axes.spines.right": False,    # 仅保留左 + 下脊线
...
# ── NMI pastel family palette (Nature Sensors submission) ─────────────
# 规则: 一个冷色家族用于 baseline，一个 hero 家族用于 this work，
# 绿色/红色仅用于方向信号（delta up/down）。
BLUE        = "#0F4D92"   # blue_main — hero method (this work)
BLUE2       = "#DCEBFF"
GRAY        = "#F5F5F3"
DARK        = "#1C1C1C"
TEXT        = "#222222"
MUTED       = "#6F6F6F"
GREEN       = "#2E9E44"   # delta_up — positive metric direction
RED         = "#E53935"   # delta_down — negative metric direction
# Palette colors for pipeline boxes
PALE_YELLOW = "#FFF7D6"
PALE_BLUE   = "#EAF3FF"
PALE_GREEN  = "#EDF7E8"
PALE_PURPLE = "#F1ECFA"
...
def arrow(ax, start, end, color="#333333", lw=1.0, ms=8, rad=0.0, zorder=7):
...
def data_unit_display_aspect(ax):
    """计算当前坐标轴里 x/y 数据单位在屏幕上的比例，保证图片不被拉伸。"""
    fig = ax.figure
    pos = ax.get_position()
...
def axes_size_mm(ax, width_mm, height_mm=None):
    """把毫米尺寸换算成当前坐标轴的数据坐标；改图片物理大小时优先改这里的输入毫米数。"""
...
def arrow_between(ax, left_ext, right_ext, y=None, gap=0.012, color="#333333", lw=1.0, ms=8, rad=0.0):
    """在两个对象外接框之间画箭头；gap 越大，箭头离图片/框越远。"""
    if left_ext is None or right_ext is None:
        return
    if y is None:
        y = ((left_ext[2] + left_ext[3]) + (right_ext[2] + right_ext[3])) / 4
    start_x = left_ext[1] + gap
    end_x = right_ext[0] - gap
    if end_x <= start_x:
        return
    arrow(ax, (start_x, y), (end_x, y), color=color, lw=lw, ms=ms, rad=rad)


def centered_gap_arrow(ax, left_ext, right_ext, y, length=0.018,
                       color="#333333", lw=1.0, ms=8, zorder=8):
    """画等长箭头；移动箭头改 y，改变箭头长度改 length，改变箭头头部大小改 ms。"""
    if left_ext is None or right_ext is None:
        return
...
def magnify_connector(fig, source_ax, source_ext, target_ax, target_span, source_pad=None):
    """画浅灰放大连接区；source_ext 控制源框，target_span 控制连接到 inset 的左右范围。"""
    # source_pad 控制源框四周留白；想让放大框贴紧图片，就用 source_pad=(0, 0)。
    # region 是 panel a 小图到下方放大 inset 的四边形；fc 控制浅灰底色。
...
def draw_image_original_ratio(ax, img, box, zorder=4, frame=False, frame_color="#B0B0B0", frame_lw=0.45, pad_frac=0.0):
    """在 box=(x, y, w, h) 中放图片并保持原始宽高比；移动图片改 x/y，放大缩小改 w/h。"""
    if img is None:
        return None
    x0, y0, w, h = box
    x0 += w * pad_frac
...
def draw_sensor_photo(ax, center, size, img_name, title=None, fallback_color="#444444"):
    """画传感器照片；center=(x,y) 控制中心位置，size=(w,h) 控制图片占用尺寸。"""
    x, y = center; w, h = size
...
def draw_raw_stack(ax, center, size):
    """画 raw_rgb 图片和后面的层叠装饰；center 改位置，size 改整体大小。"""
    x, y = center; w, h = size
...
def draw_depth(ax, extent, title=None, show_scale=False):
    """画深度图；extent=(x0, x1, y0, y1) 同时决定位置和大小，title 控制下方/上方文字。"""
...
def solver_card(ax, center, size, title, stages, footer, accent, fill):
    """panel a 第三项流程卡；center 改位置，size 改卡片大小，stages 改三个小框文字。"""
    x, y = center
    w, h = size
    ...
    stage_y = y+h*0.01
    # 三个内部小框：stage_w 越小，框间箭头越清楚；stage_gap 越大，箭头越长。
    stage_w = w * 0.205
    stage_h = h * 0.23
    stage_gap = w * 0.115
...
def metric_note(ax, center, size, heading, detail, color):
    """panel a 第五项性能对比；center 改位置，size 改文字区域大小，detail 改三行内容。"""
    x, y = center
    w, h = size
...
def leader_label(ax, anchor, label_y, text, side="right"):
    """爆炸图注解引线；anchor 是结构真实高度，label_y 是文字高度，text_x 控制标签左右位置。"""
    x, y = anchor
    if label_y is None:
        label_y = y
    if side == "right":
...
def pipeline_box(ax, xy, wh, title, detail, fill, edge="#B7B7B7", title_color=TEXT):
    """画 panel c 的单个 pipeline 模块；xy 是左下角，wh 是宽高，title/detail 是上下两行文字。"""
    x, y = xy
    w, h = wh
...
def image_placeholder(ax, box, title, note=None):
    """画缺图时的占位区域；box=(x,y,w,h) 可直接控制占位图位置和大小。"""
    x, y, w, h = box
    ax.add_patch(Rectangle((x, y), w, h, fc="#FAFBFC", ec="none",
                           lw=0, zorder=2))
...
def pixel_stream_icon(ax, box):
    """画 CMOS 像素小方格；box=(x,y,w,h) 控制位置和大小，colors 控制每个像素颜色。"""
    x, y, w, h = box
    colors = [
...
def span_label(ax, start, end, y, text):
    """画 pipeline 下方的双向范围箭头；start/end 控制左右端点，y 控制高度，text 控制说明文字。"""
...
def draw_panel_a(ax):
    """panel a：上方 host reconstruction 与下方 near-sensor this work 的对比流程图。"""
    hide_ax(ax)
    ...
    # xs 控制五个流程节点的横向位置：传感器、raw RGB、solver、depth、指标说明。
    # 想把某一列左/右移动，就改对应 xs 的数字。
    xs = [0.100, 0.270, 0.505, 0.705, 0.890]
    # yt 是 host 行纵向位置，yb 是 this-work 行纵向位置；数值越大越靠上。
    yt, yb = 0.750, 0.200
    # 这里控制两行引用图片的物理尺寸；目前为 10 mm x 10 mm。
    # 想让所有小图片变大/变小，改这两个 10.0。
    img_size = axes_size_mm(ax, 10.0, 10.0)
    host_img_size = img_size
    img_w, img_h = img_size
    host_img_w, host_img_h = host_img_size

    # 上方流程：传统 host reconstruction。
    # 修改某张图片：改文件名，例如 GelSight.png；修改图片位置：改 xs 或 yt。
    top_exts = []
    ...
    # 下方流程：this work，突出 near-sensor on-chip pipeline。
    # 修改下方传感器图片：改 Sensor.png；修改浅蓝底色高度：看 extend_down_5mm。
    bottom_exts = []
    ...
    # 浅蓝底色下边距：5 mm。想让浅蓝底色向下延伸/缩短，改这里的 5.0。
...
def draw_panel_b(ax, label="b"):
    """panel a 下方左侧 inset：传感器爆炸图；图片位置改 draw_image_original_ratio 的 box。"""
    hide_ax(ax)
    ...
    ih = iy1 - iy0
    # label_ys 控制每个标签文字的纵向位置；np.linspace(上, 下, 数量) 可整体压缩/拉开间距。
    # part_marks 每一项为 (结构高度比例, 标签y, 标签文字)：
    # 第一个数控制虚线从爆炸图哪个高度引出，第二个数控制文字放在哪个高度。
...
def draw_panel_c(ax, label="c"):
    """panel a 下方右侧 inset：片上 pipeline 展开图；模块位置和大小主要由 stage_specs 控制。"""
    hide_ax(ax)
    panel_label(ax, label, x=-0.028, y=1.015)
    # CMOS pixels 方块大小为 8 mm x 8 mm；改 8.0 可放大/缩小输入像素图。
    pixel_w, pixel_h = axes_size_mm(ax, 8.0, 8.0)
    # pixel_x 控制输入像素图左右位置；0.555 是 pipeline 中心高度。
    ...
    # stage_specs 每一项为 (模块宽度, 标题, 细节, 填充色, 边框色)。
    # 想让某个模块更宽/更窄，改第一列；想改文字，改标题或细节字符串。
    stage_specs = [
    ...
    # depth_x 控制右侧 depth out 图片的横向位置。
    depth_x = 0.895
    ...
    for w, title, detail, fill, edge in stage_specs:
        ...
        stage_x += w + stage_gap
    # stage_y 控制模块底部高度，stage_h 控制模块高度。
...
def draw_press_schematic(ax, cx, y, w, h, progress):
    """备用按压示意图；cx/y 控制位置，w/h 控制大小，progress 控制压入程度。"""
    x0, x1 = cx - w/2, cx + w/2
    gel_y = y + h*0.34
    base_y = y + h*0.12
    ax.add_patch(Rectangle((x0+w*0.16, base_y), w*0.68, h*0.16,
                           fc="#EEF2F5", ec="#AAB4BD", lw=0.38, zorder=2))
    surf_x = np.array([x0+w*0.14, cx-w*0.16, cx, cx+w*0.16, x1-w*0.14])
    dent = h * 0.10 * progress
    surf_y = np.array([gel_y, gel_y, gel_y-dent, gel_y, gel_y])
    ax.plot(surf_x, surf_y, color="#4B5560", lw=0.55, zorder=3)
    ax.fill_between(surf_x, surf_y, base_y+h*0.15, color="#DDE8F0", alpha=0.85, zorder=1)
    radius = h * 0.145
    ball_y = y + h*(0.84 - 0.30*progress)
    ax.add_patch(Circle((cx, ball_y), radius, fc="#8D969E", ec="#58616A",
                        lw=0.42, zorder=4))
    ax.add_patch(FancyArrowPatch((cx, y+h*0.96), (cx, ball_y+radius*1.25),
                                 arrowstyle="-|>", mutation_scale=4.5,
                                 lw=0.45, color="#58616A", zorder=4,
                                 shrinkA=0, shrinkB=0))


def draw_panel_e(ax):
    """panel f：按压时间序列；times/stages/descriptions 控制每列上中下三处文字。"""
    hide_ax(ax)
    panel_label(ax, "f", x=-0.035, y=1.015)
    # times 是每列上方时间戳；stages 是两排图片中间的阶段名。
    # descriptions 是每列最下方的简短说明。
    ...
    # 每张深度图/按压图大小为 12 mm x 12 mm；改这里可以统一缩放所有时间帧。
    w, h = axes_size_mm(ax, 12.0, 12.0)
    gap = (0.920 - 6*w) / 5
    # positions 控制上排深度图的位置；0.040 是第一张图的 x，0.520 是上排 y。
...
def make_figure():
    ...
    # 手动设置每个 panel 的位置，保证 PDF/PNG 导出稳定。
    # fig.add_axes([left, bottom, width, height]) 均为整张画布的 0-1 比例坐标：
    # left/bottom 控制 panel 左下角位置，width/height 控制 panel 宽高。
    # 想移动某个 panel：改前两个数；想放大/缩小某个 panel：改后两个数。
    ax_a = fig.add_axes([0.040, 0.800, 0.650, 0.160])
    ax_b = fig.add_axes([0.040, 0.575, 0.205, 0.190])
    ax_c = fig.add_axes([0.265, 0.575, 0.425, 0.190])
    ax_app = fig.add_axes([0.730, 0.615, 0.230, 0.345])
    ax_d = fig.add_axes([0.060, 0.298, 0.220, 0.257], projection="polar")
    ax_e = fig.add_axes([0.335, 0.301, 0.625, 0.239])
    focus = draw_panel_a(ax_a)
    draw_panel_b(ax_b, label="b")
    draw_panel_c(ax_c, label="c")
    draw_panel_application(ax_app)
    draw_panel_radar(ax_d)
    draw_panel_e(ax_e)
    # 浅灰连接区表示下方 inset 是 panel a 中对应对象的放大。
    # 如果连接区左右没对齐，改 magnify_connector 的 source_pad 或 target_span。
    magnify_connector(fig, ax_a, focus["sensor"], ax_b, (0.000, 1.000), source_pad=(0.000, 0.000))
    magnify_connector(fig, ax_a, focus["pipeline"], ax_c, (0.000, 1.000))
    return fig


if __name__ == "__main__":
    fig = make_figure()
    base = OUT_DIR / "Fig1_nature_sensors_artguide"
    # SVG — 主格式：矢量无损，文字可编辑（Illustrator / Inkscape）
    fig.savefig(f"{base}.svg", bbox_inches="tight", pad_inches=0.02)
    # PDF — 投稿必需
    fig.savefig(f"{base}.pdf", bbox_inches="tight", pad_inches=0.02)
    # TIFF — Nature 系列常见要求，600 dpi
    fig.savefig(f"{base}.tiff", dpi=600, bbox_inches="tight", pad_inches=0.02)
    # PNG — 快速预览
