...
def load_panel_d_pareto():
    df = fill_operating_metrics(read_csv(FILE_D))
...
def load_panel_e_scaling():
    df = fill_operating_metrics(read_csv(FILE_E))
    if df['latency_ms'].dropna().empty or df['power_w'].dropna().empty:
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v7.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v7.png'
