...
def draw_panel_d(ax, df):
    ...
    # Log-scaled throughput axis. Keep the left limit positive; 0 cannot be used on log axes.
    positive_x = [float(x) for x in xvals if float(x) > 0]
    xmin = min(25.0, min(positive_x) * 0.85) if positive_x else 25.0
    xmax = max(450.0, max(positive_x) * 1.28) if positive_x else 450.0
    ax.set_xscale('log')
    ...
    # Direct labels near the highest-throughput points; multiplicative offsets are more stable on log axes.
    label_spec = {
        'ASIC full-chip': (1.05, 1.03),
        'Jetson Orin NX': (1.06, 1.00),
        'CPU i7-13700': (1.05, 1.02),
        'RTX 4090': (1.05, 0.96),
    }
    for name, x, y in endpoint_rows:
        xmul, ymul = label_spec.get(name, (1.05, 1.0))
        ax.text(x * xmul, y * ymul, name, fontsize=4.9, color=color_map[name], ha='left', va='center')
    ...
    # Nature-style sparse ticks on the log throughput axis.
    tick_candidates = np.array([30, 60, 120, 180, 240, 400, 800, 1200], dtype=float)
    ticks = tick_candidates[(tick_candidates >= xmin) & (tick_candidates <= xmax)]
    ax.set_xticks(ticks)
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v14.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v14.png'
