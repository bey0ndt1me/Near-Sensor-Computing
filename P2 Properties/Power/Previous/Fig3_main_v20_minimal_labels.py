...
def draw_panel_c(ax, df):
    ...
    for i, p in enumerate(plot_order):
        ...
        total = float(row['core_power_w']) + float(row['background_power_w'])
        ax.bar(x[i], total - y_base, bottom=y_base, width=0.48,
               color=colors[i], edgecolor='white', linewidth=0.6, alpha=0.92)
...
def main():
    ...
    add_caption(fig)
    out_pdf = SCRIPT_DIR / 'Fig3_main_v20_minimal_labels.pdf'
    out_png = SCRIPT_DIR / 'Fig3_main_v20_minimal_labels.png'
